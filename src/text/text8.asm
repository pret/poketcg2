Text0c95:
	textfw "さあ つぎは"
	linefw "水のGR団のトリデに むかうといい"
	done

Text0c96:
	textfw "もう もっているんだな"
	linefw "「水のGR団コイン」は・・・"
	linefw "それならば 闘のGR団のトリデに"
	linefw "むかうと いい"
	done

Text0c97:
	textfw "ちょっと まって! あたらしい"
	linefw "りょうりを かんがえてるんだ"
	linefw "うーん きみが いると"
	linefw "きが ちってしょうがないや・・・"
	linefw "いいや かんがえるの"
	linefw "やめた!"
	linefw "ぼくは GR団員のジェス!"
	linefw " "
	linefw "ヒデロウさんの めいれいで"
	linefw "きみを りょうりしなきゃいけないんだ"
	done

Text0c98:
	textfw "さっさと 対戦してくれよ!"
	linefw "ぼくは いそがしいんだ!"
	linefw "ぼやぼやしてると"
	linefw "ようがんに つきおとすよ!"
	done

Text0c99:
	textfw "ジェスと 対戦しますか?"
	done

Text0c9a:
	textfw "4枚のサイドカード対戦で"
	linefw "さっさと しよう!"
	done

Text0c9b:
	textfw "ぼくは あたらしいりょうりを"
	linefw "かんがえたいんだ"
	linefw "だから 対戦しなくても"
	linefw "ぜーんぜん かまわないよ"
	linefw "こまるのは きみだしね"
	done

Text0c9c:
	textfw "あたらしいりょうりを いくつか"
	linefw "かんがえたから 対戦しても いいよ"
	done

Text0c9d:
	textfw "サイドカードは 4枚"
	done

Text0c9e:
	textfw "あっそ じゃあ ぼくは"
	linefw "りょうりを かんがえよっと"
	done

Text0c9f:
	textfw "ええと ぼくが まけたから"
	linefw "まずは 拡張パックだっけ"
	done

Text0ca0:
	textfw "あーあ 勝っちゃったよ"
	linefw "ぼくいそがしいんだよ"
	linefw "さっさと 対戦して"
	linefw "勝ってよね"
	done

Text0ca1:
	textfw "勝ったきみには 拡張パック"
	done

Text0ca2:
	textfw "ん まあ きみとの 対戦は"
	linefw "いがいと たのしいかな"
	linefw "よかったら また 対戦しようか"
	done

Text0ca3:
	textfw "さあ 対戦も おわったから"
	linefw "りょうりを かんがえよっと"
	done

Text0ca4:
	textfw "ユウキちゃんのへやの とびらを"
	linefw "いま あけるよ"
	linefw "このいわの スイッチを"
	linefw "おすと あくんだ"
	done

Text0ca5:
	textfw "これで さきに すすめるよね"
	linefw "さっさと いくと いいよ"
	done

Text0ca6:
	textfw "わたしは・・ショウコ・・GR団員・・"
	linefw " "
	linefw "あなたは・・わたしと・・しょうぶ・・"
	linefw "しなければ・・・いけない"
	linefw "勝てば・・・あのオンナ・・・"
	done

Text0ca7:
	textfw "グランドマスター・・・ヒロコ・・・"
	linefw "かえして・・・あげる・・・"
	done

Text0ca8:
	textfw "しょうぶは・・・じょうけんつき・・・"
	linefw "イーブイ・・・4枚・・・いれなさい"
	linefw "そうすれば・・・しょうぶする・・・"
	done

Text0ca9:
	textfw "4枚のイーブイ・・・デッキに・・・"
	linefw "いれなさい・・・"
	linefw "そうすれば・・・しょうぶ・・する"
	done

Text0caa:
	textfw "ショウコと 対戦しますか?"
	done

Text0cab:
	textfw "イーブイ・・・たりない"
	linefw "デッキに・・いれてから・・きなさい"
	done

Text0cac:
	textfw "サイドカード・・・4枚・・・"
	done

Text0cad:
	textfw "すきに・・・しなさい・・・"
	done

Text0cae:
	textfw "4枚のイーブイ・・・デッキに・・・"
	linefw "いれなさい・・・"
	linefw "そうすれば・・・しょうぶ・・する"
	done

Text0caf:
	textfw "・・・まけました・・・"
	done

Text0cb0:
	textfw "あなた・・・つよいわ・・・"
	done

Text0cb1:
	textfw "フッ・・・"
	linefw "わたしの・・・勝ち"
	done

Text0cb2:
	textfw "あなた・・・すごく・・・つよい"
	done

Text0cb3:
	textfw "デッキを・・・つかうのも・・"
	linefw "とっても・・じょうず・・・"
	done

Text0cb4:
	textfw "トビラは・・・あかないわ・・・"
	linefw "あたしに・・かたないと・・・"
	done

Text0cb5:
	textfw "・・・ヒロコ・・・かえす・・・"
	done

Text0cb6:
	textfw "グランドマスターの ヒロコが"
	linefw "GR団につかまるなんて ふがいないわ"
	linefw "でも・・・ウフフ・・・うれしいわ"
	linefw "<RAMNAME>くんに たすけてもらって"
	linefw "うれしいから おれいに"
	linefw "カードを あげるわね"
	done

Text0cb7:
	textfw "これからの 対戦につかってネ"
	linefw "GR団は つよいわ きをつけて!"
	linefw "ウフフ・・・アタシは"
	linefw "ポケモンドームに かえるわね"
	linefw "ポケモンドームで・・・ウフフ・・"
	linefw "まってるわ"
	done

Text0cb8:
	textfw "・・・ヒデロウさまに・・・あえ・・"
	done

Text0cb9:
	textfw "・・いけ・・"
	done

Text0cba:
	textfw "ワシは GR団員の おじーさんじゃ"
	linefw "じゃから みな おGさんと よぶ!"
	linefw "じゃが! このごろの わかいモンは"
	linefw "なっとらーん!!"
	linefw "デッキを だいじにしすぎて"
	linefw "カードのこうかんすら しようとせん!"
	linefw "そのせいで デッキをつくりたくとも"
	linefw "カードが たりゃせんわ!"
	linefw "おまえさんは GR団員じゃ"
	linefw "ないじゃろ?"
	linefw "ワシに「わるいリザードン<Lv>38」を"
	linefw "ゆずって くれんか?"
	linefw "かわりに 「リザードン<Lv>76」を"
	linefw "ワシのカードから プレゼントじゃ"
	linefw "めずらしい 「リザードン<Lv>76」じゃ"
	linefw "どうじゃな? こうかんせんか?"
	done

Text0cbb:
	textfw "おまえさんは GR団に ひとりで"
	linefw "さからっとる そうじゃな"
	linefw "ひとりで たたかうとは・・・"
	linefw "おおいに きにいったぞ!!"
	linefw "ぜひとも ワシのカードの"
	linefw "「リザードン<Lv>76」と"
	linefw "「わるいリザードン<Lv>38」を"
	linefw "こうかんしてくれぃ!"
	done

Text0cbc:
	textfw "ワシは おまえさんと カードを"
	linefw "こうかんすると きめたんじゃ"
	linefw "だれとも こうかんせんで"
	linefw "まっとるよ"
	done

Text0cbd:
	textfw "ううむ 「わるいリザードン<Lv>38」は"
	linefw "もっとらんのじゃな"
	linefw "ワシは おまえさんとしか"
	linefw "こうかんするきが ないんじゃ"
	linefw "ぜひ 「わるいリザードン<Lv>38」を"
	linefw "さがして こうかんしてくれ"
	done

Text0cbe:
	textfw "いくら ワシでも おまえさんの"
	linefw "デッキから カードは とれん"
	linefw "たたかいに そなえて"
	linefw "デッキを だいじにするんじゃ!"
	done

Text0cbf:
	textfw "おお! 「わるいリザードン<Lv>38」を"
	linefw "もっとるんじゃな!"
	linefw "では こうかんと いこう"
	done

Text0cc0:
	textfw "いまのGR団は デッキに"
	linefw "こだわり すぎなんじゃ"
	linefw "おまえさんと たたかうことで"
	linefw "きがつくと ええんじゃが・・・"
	done

Text0cc1:
	textfw "おまえさんとの たたかいで"
	linefw "GR団も すこしずつ かわっておる"
	linefw "ええことじゃ"
	done

Text0cc2:
	textfw "GR団は つよい!"
	linefw "GR団は ただしい!"
	linefw "カードは デッキに いれて"
	linefw "つかってこそ かちがあるんだー!!"
	linefw "・・・って GR団員は いってるんだ"
	linefw "きみ GR団員じゃないよね"
	linefw "ほんとはね ボクは カードを"
	linefw "あつめても いいと おもってるんだ"
	linefw "だって たのしみかたは"
	linefw "ひとそれぞれだもんね"
	linefw "GR団が うるさいから"
	linefw "このことは ヒミツだよ"
	done

Text0cc3:
	textfw "ビルリッチさまが まけたから"
	linefw "やっと どーどーと いえるよ"
	linefw "ボクは カードを あつめるのも"
	linefw "対戦するのも だいすきだーーー!!!"
	done

Text0cc4:
	textfw "まってたよー!"
	linefw "<RAMNAME>ちゃん!"
	linefw "えーと・・・それから・・・"
	linefw "なんだっけ?"
	done

Text0cc5:
	textfw "(えーと まずは なまえをいって"
	linefw " それから 対戦・・・)"
	done

Text0cc6:
	textfw "アタシ"
	linefw "GR団員のユウキだよ!"
	linefw "<RAMNAME>ちゃんと りょうり・・・"
	linefw "じゃなくて 対戦!"
	linefw "<RAMNAME>ちゃんと 対戦するの!"
	linefw " "
	linefw "勝ったら"
	linefw "拡張パックを 1つ あげまーす!"
	done

Text0cc7:
	textfw "対戦 対戦 たいせぇーん!"
	linefw "・・・するんだよね?"
	linefw "えーと アタシとの対戦は・・・"
	linefw "なんだっけ?"
	done

Text0cc8:
	textfw "(いれていいカードは ・・・"
	linefw " 炎エネルギーだけ・・・)"
	done

Text0cc9:
	textfw "アタシとの 対戦は"
	linefw "じょうけんつきでーす!"
	linefw "いれていい エネルギーカードは"
	linefw "炎エネルギーだけでーす!"
	linefw "デッキのじゅんびは"
	linefw "いーですか?"
	done

Text0cca:
	textfw "アタシとの 対戦は"
	linefw "じょうけんつきでーす!"
	linefw "えっと じょうけん・・・"
	linefw "なんだっけ・・・"
	done

Text0ccb:
	textfw "(いれていいカードは・・・"
	linefw " 炎エネルギー・・・)"
	done

Text0ccc:
	textfw "いれていい エネルギーカードは"
	linefw "炎エネルギーだけ だよ"
	linefw "対戦に 勝てば"
	linefw "拡張パックを あげまーす!"
	done

Text0ccd:
	textfw "ユウキと 対戦しますか?"
	done

Text0cce:
	textfw "あれ ほかのエネルギーが"
	linefw "はいってる・・・"
	done

Text0ccf:
	textfw "(ほかのエネルギーが はいってたら"
	linefw " 炎だけに してから きてもらう)"
	done

Text0cd0:
	textfw "いれていい エネルギーカードは"
	linefw "炎エネルギーだけ だよ"
	linefw "炎エネルギーだけ いれて"
	linefw "また きてくださーい"
	done

Text0cd1:
	textfw "サイドカードは 4枚でーす!"
	done

Text0cd2:
	textfw "対戦しないの?"
	linefw "そのばあいは・・・"
	done

Text0cd3:
	textfw "(対戦をことわられたら・・・"
	linefw " また くるように はなす・・・)"
	done

Text0cd4:
	textfw "また 対戦しに きてください"
	done

Text0cd5:
	textfw "あっ? <RAMNAME>ちゃん! えっと"
	linefw "いっかい まけたあとの対戦は・・・"
	done

Text0cd6:
	textfw "(いままでと かわりなく"
	linefw " 対戦をすること・・・)"
	done

Text0cd7:
	textfw "アタシとの 対戦は"
	linefw "じょうけんつきでーす!"
	linefw "いれていい エネルギーカードは"
	linefw "炎エネルギーだけ だよ"
	linefw "対戦に 勝てば"
	linefw "拡張パックを あげまーす!"
	done

Text0cd8:
	textfw "ほかのエネルギーが"
	linefw "はいってまーす"
	linefw "炎エネルギーだけ いれて"
	linefw "また きてくださーい"
	done

Text0cd9:
	textfw "また 対戦しに きてください"
	done

Text0cda:
	textfw "えーと まけたら 拡張パックを"
	linefw "わたすんだよね"
	done

Text0cdb:
	textfw "あれ? アタシが 勝っちゃった"
	linefw "そのばあいは・・・"
	done

Text0cdc:
	textfw "(勝ったばあいは・・・"
	linefw " 勝ったことを いばる・・・)"
	done

Text0cdd:
	textfw "どうだ 勝ったぞー"
	linefw "エライでしょう! エッヘン!"
	linefw "まけたから <RAMNAME>ちゃんは"
	linefw "おかえりくださーい"
	done

Text0cde:
	textfw "拡張パックを わたしまーす!"
	done

Text0cdf:
	textfw "(2かいめからは また対戦に"
	linefw " くるように いうこと・・・)"
	done

Text0ce0:
	textfw "また 対戦に きてくださーい"
	done

Text0ce1:
	textfw "まけたから <RAMNAME>ちゃんは"
	linefw "おかえりくださーい"
	done

Text0ce2:
	textfw "えーと 対戦で まけたばあいは・・・"
	done

Text0ce3:
	textfw "(さいしょのときは トビラを"
	linefw " あける・・・)"
	done

Text0ce4:
	textfw "えーと ショウコさんのへやの"
	linefw "トビラを あけるね!"
	done

Text0ce5:
	textfw "トビラ あけー!"
	done

Text0ce6:
	textfw "じゃあ ショウコさんに あってね!"
	done

PawnWantsToDuelInitialText:
	textfw "ボクは ポーン"
	linefw "ダンジョンさいしょの対戦あいてさっ!"
	linefw "ボクと対戦して 勝てば"
	linefw "さらに さきに すすめるんだっ"
	linefw "ただし 対戦するなら"
	linefw "チップを 10枚もらうよ"
	linefw "もちろん あなたが勝てば"
	linefw "チップを20枚にして かえしてあげる"
	linefw "チップを 10枚 はらって"
	linefw "対戦する?"
	done

PawnWantsToDuelRepeatText:
	textfw "ボクと対戦して 勝てば"
	linefw "さきに すすむことができるよ"
	linefw "でも 対戦には チップが10枚"
	linefw "ひつようなんだ それでも対戦する?"
	done

PawnDuelPromptText:
	textfw "ポーンと 対戦しますか?"
	done

PawnDuelStartText:
	textfw "ボクとの 対戦は"
	linefw "サイドカード 4枚さっ!"
	linefw "それじゃ 対戦しよう!"
	done

PawnDeclinedDuelText:
	textfw "対戦しないの?"
	linefw "ほんとうに いいんだね?"
	done

PawnQuitDuelPromptText:
	textfw "ポーンとの 対戦をやめますか?"
	done

PawnResumeDuelText:
	textfw "それなら ボクと"
	linefw "チップをかけて 対戦するんだね?"
	done

PawnPlayerQuitText:
	textfw "では では ダンジョンから"
	linefw "おひきとりくださーい"
	done

PawnProceedRepeatText:
	textfw "はーい どうぞ!"
	linefw "さきに すすんで いいよっ!"
	done

PawnPlayerWon1Text:
	textfw "おーみーごーとー"
	linefw "じゃあ チップを かえすね!"
	done

PawnPlayerWon2Text:
	textfw "ボクに 勝ったから"
	linefw "さきに すすめるよっ!"
	linefw "べつに ダンジョンから でてもいいし"
	linefw "さきに すすんでも いいのさっ!"
	linefw "どうする?"
	done

PawnPlayerLostText:
	textfw "キミの まけだね"
	linefw " "
	linefw "まけたひとは"
	linefw "ダンジョンから でてもらうよっ!"
	done

PawnProceedWithCardDungeonPromptText:
	; each Card Dungeon opponent has the same ProceedWithCardDungeonPromptText
	textfw "ダンジョンを さきに すすみますか?"
	done

PawnProceedInitial1Text:
	textfw "じゃあ おくのトビラを"
	linefw "あけてあげるね!"
	done

PawnProceedInitial2Text:
	textfw "はーい どうぞ!"
	linefw "さきに すすんでいいよっ!"
	done

PawnDeclinedProceedingText:
	textfw "さきに すすまないなら"
	linefw "ダンジョンから でてもらうよっ!"
	linefw "それでも いいのかな?"
	done

PawnWithdrawFromCardDungeonPromptText:
	; each Card Dungeon opponent has the same WithdrawFromCardDungeonPromptText
	textfw "ダンジョンから でますか?"
	done

PawnPlayerWithdrewText:
	textfw "じゃあ さようならだねっ!"
	done

KnightWantsToDuelInitialText:
	textfw "それがしは ここをまもる"
	linefw "ナイトで ござるよ"
	linefw "それがしと 対戦をして"
	linefw "勝てれば さきにすすめるのでござる"
	linefw "対戦するなら チップが"
	linefw "10枚か 20枚いるのでござる"
	linefw "勝ったばあいは 10枚なら 20枚"
	linefw "20枚なら 40枚で かえすでござる"
	linefw "いかがか?"
	linefw "それがしと 対戦するでござるか?"
	done

KnightWantsToDuelRepeatText:
	textfw "さきに すすむには それがしと"
	linefw "対戦して 勝たねば ならんでござる"
	linefw "対戦するならば チップを"
	linefw "10枚か 20枚だすでござる"
	linefw "勝ったばあいは 10枚なら 20枚"
	linefw "20枚なら 40枚で かえすでござる"
	linefw "いかがか?"
	linefw "それがしと 対戦するでござるか?"
	done

KnightDuelPromptText:
	textfw "ナイトと 対戦しますか?"
	done

KnightDuelStartText:
	textfw "わかった <RAMNUM>枚で"
	linefw "対戦するでござるな"
	linefw "それがしとの 対戦は サイドカード"
	linefw "4枚でおねがいするでござる"
	linefw "では"
	linefw "いざ 対戦でござる!"
	done

KnightDeclinedDuelText:
	textfw "なんと! 対戦なさらないと"
	linefw "いわれるでござるか?"
	linefw "対戦しなければ ダンジョンから"
	linefw "でてしまうでござるよ"
	linefw "しかも いちどでも でてしまうと"
	linefw "さいしょから やりなおしでござる"
	done

KnightQuitDuelPromptText:
	textfw "ナイトとの 対戦をやめますか?"
	done

KnightResumeDuelText:
	textfw "では"
	linefw "対戦するでござるか?"
	done

KnightPlayerQuitText:
	textfw "では さらばでござる"
	done

KnightProceedRepeatText:
	textfw "つぎの対戦あいては"
	linefw "ビショップどのでござる"
	linefw "ビショップどのは"
	linefw "おつよいので がんばるでござる"
	done

KnightPlayerWon1Text:
	textfw "おめでとうでござる!"
	linefw "チップを おわたしするでござる"
	done

KnightPlayerWon2Text:
	textfw "さきに すすむでござる"
	linefw "それとも やめるでござるか?"
	done

KnightPlayerLostText:
	textfw "そなたの まけでござる"
	linefw "ダンジョンから でてもらうでござる"
	done

KnightProceedWithCardDungeonPromptText:
	textfw "ダンジョンを さきに すすみますか?"
	done

KnightProceedInitial1Text:
	textfw "それでは"
	linefw "トビラを あけるでござる"
	done

KnightProceedInitial2Text:
	textfw "ささ"
	linefw "おくのヘヤに すすむでござる"
	done

KnightDeclinedProceedingText:
	textfw "さきに すすまないのならば"
	linefw "ダンジョンから でてしまうでござる"
	linefw "それでも よいでござるか?"
	done

KnightWithdrawFromCardDungeonPromptText:
	textfw "ダンジョンから でますか?"
	done

KnightPlayerWithdrewText:
	textfw "対戦しないのならば"
	linefw "ダンジョンから でてもらうでござる"
	done

BishopWantsToDuelInitialText:
	textfw "ワシは ビショップさまじゃ!"
	linefw "カンタンには さきに いかさんぞ!"
	linefw "ワシと 対戦したいなら チップを"
	linefw "10枚か 30枚 わたすんじゃ!"
	linefw "勝てたら さきに いかせるし"
	linefw "チップも ふやしてかえすぞ!"
	linefw "30枚なら 60枚にして"
	linefw "かえしてやろう"
	linefw "どうじゃ? チップを わたして"
	linefw "対戦するかい?"
	done

BishopWantsToDuelRepeatText:
	textfw "チップを ふやしたいなら"
	linefw "対戦すべきじゃ"
	linefw "対戦するなら もちろん チップは"
	linefw "はらってもらうぞ"
	done

BishopDuelPromptText:
	textfw "ビショップと 対戦しますか?"
	done

BishopNotEnoughChipsText:
	textfw "チップが たりんようじゃな"
	linefw "チップを ふやしてからくるんじゃな"
	done

BishopDuelStartText:
	textfw "<RAMNUM>枚じゃな?"
	linefw "よーし 対戦かいしじゃ!"
	linefw "サイドカードは 4枚じゃ!"
	done

BishopDeclinedDuelText:
	textfw "対戦せんのか?"
	linefw "チップを ふやさなくていいんじゃな?"
	done

BishopQuitDuelPromptText:
	textfw "ビショップとの 対戦をやめますか?"
	done

BishopResumeDuelText:
	textfw "対戦したくなったんじゃな?"
	done

BishopPlayerQuitText:
	textfw "それなら ダンジョンから"
	linefw "でてもらおうかの"
	done

BishopProceedRepeatText:
	textfw "つぎの対戦は ダンジョンに"
	linefw "はいりなおしたときじゃ"
	done

BishopPlayerWon1Text:
	textfw "おまえの 勝ちじゃ!"
	linefw "ヤクソクじゃ チップを わたすぞ"
	done

BishopPlayerWon2Text:
	textfw "ワシに 勝ったんじゃ"
	linefw "トビラを あけるが どうする?"
	done

BishopPlayerLostText:
	textfw "おまえのまけじゃ!"
	linefw "ダンジョンから でてもらうぞ!"
	done

BishopProceedWithCardDungeonPromptText:
	textfw "ダンジョンを さきに すすみますか?"
	done

BishopProceedInitial1Text:
	textfw "よーし トビラを あけるぞ!"
	done

BishopProceedInitial2Text:
	textfw "さあ さきに すすむんじゃ!"
	done

BishopDeclinedProceedingText:
	textfw "では ダンジョンから"
	linefw "でていってもらおう"
	done

BishopWithdrawFromCardDungeonPromptText:
	textfw "ダンジョンから でますか?"
	done

BishopPlayerWithdrewText:
	textfw "それなら ダンジョンから"
	linefw "でてもらおうかの!"
	done

RookWantsToDuelInitialText:
	textfw "オレは ルークだ"
	linefw "よく ここまできたな"
	linefw "おくに いらっしゃる クィーンさまに"
	linefw "おあいしたくば オレに勝つんだ"
	linefw "対戦するなら チップを"
	linefw "30枚か 50枚 よこすんだ"
	linefw "わかったか? チップを よこして"
	linefw "オレと 対戦するのか?"
	done

RookWantsToDuelRepeatText:
	textfw "クィーンさまに おあいしたくば"
	linefw "オレと たたかうのだ"
	linefw "対戦するなら チップを"
	linefw "30枚か 50枚 よこせ"
	linefw "わかったか? チップを わたして"
	linefw "オレと 対戦するのか?"
	done

RookDuelPromptText:
	textfw "ルークと 対戦しますか?"
	done

RookNotEnoughChipsText:
	textfw "あっはははは・・・からかうなよ"
	linefw "こまったもんだ"
	linefw "チップが たりないぞ"
	done

RookDuelStartText:
	textfw "<RAMNUM>枚だな"
	linefw "対戦を うけてたつぞ"
	linefw "オレは サイドカード"
	linefw "5枚で たたかうぞ"
	done

RookDeclinedDuelText:
	textfw "ほんとうに 対戦しないんだな?"
	done

RookQuitDuelPromptText:
	textfw "ルークとの 対戦をやめますか?"
	done

RookResumeDuelText:
	textfw "ほんとうに 対戦するのだな?"
	done

RookPlayerQuitText:
	textfw "では ダンジョンから"
	linefw "でてもらおう!"
	done

RookProceedRepeatText:
	textfw "クィーンさまに 勝てれば"
	linefw "このダンジョンに テキなしだ"
	done

RookPlayerWon1Text:
	textfw "いさぎよく みとめよう"
	linefw "オレのまけだ・・・"
	done

RookPlayerWon2Text:
	textfw "どうするのだ?"
	linefw "クィーンさまと たたかうのか?"
	done

RookPlayerLostText:
	textfw "オレの勝ちだ"
	linefw " "
	linefw "まけたのだから ダンジョンから"
	linefw "でていって もらうぞ"
	done

RookProceedWithCardDungeonPromptText:
	textfw "ダンジョンを さきに すすみますか?"
	done

RookProceedInitial1Text:
	textfw "かくごは できてるようだから"
	linefw "トビラを ひらくぞ!"
	done

RookProceedInitial2Text:
	textfw "このさきに クィーンさまが いる"
	linefw "こころして いけ!"
	done

RookDeclinedProceedingText:
	textfw "それならば ダンジョンから"
	linefw "でるしかないが いいのだな"
	done

RookWithdrawFromCardDungeonPromptText:
	textfw "ダンジョンから でますか?"
	done

RookPlayerWithdrewText:
	textfw "それならば ここよりさるが いい"
	done

QueenWantsToDuelInitialText:
	textfw "おお!"
	linefw "チャレンジャーじゃな"
	linefw "なかなか ワラワのところまで"
	linefw "たどりつくものが おらんでな"
	linefw "まちきれんで 対戦テーブルで"
	linefw "まって おったのじゃ"
	linefw "ワラワが"
	linefw "さいごの対戦あいて クィーンじゃ"
	linefw "ざんねんながら キングは"
	linefw "キャッスルに いらっしゃる"
	linefw "じゃから ここでは"
	linefw "ワラワが おあいてするぞえ"
	linefw "ワラワの対戦は チップを"
	linefw "50枚か 100枚じゃ"
	linefw "すこし チップの枚すうが"
	linefw "おおいが 対戦せんかえ?"
	done

QueenWantsToDuelRepeatText:
	textfw "ワラワの対戦は チップを"
	linefw "50枚か 100枚じゃよ"
	linefw "すこし チップの枚すうが"
	linefw "おおいが 対戦せんかえ?"
	done

QueenDuelPromptText:
	textfw "クィーンと 対戦しますか?"
	done

QueenNotEnoughChipsText:
	textfw "それでは"
	linefw "チップが たりんぞえ"
	done

QueenDuelStart1Text:
	textfw "対戦を うけるのじゃな!"
	linefw "よい こころがけじゃ"
	linefw "それでは そちらのイスに"
	linefw "かけるが よかろう"
	done

QueenDuelStart2Text:
	textfw "サイドカードは 6枚じゃ"
	linefw "さあ たたかうぞえ!!"
	done

QueenDeclinedDuelText:
	textfw "ワラワとの 対戦をやめて"
	linefw "ダンジョンから でるのかえ?"
	done

QueenQuitDuelPromptText:
	textfw "クィーンとの 対戦をやめますか?"
	done

QueenResumeDuelText:
	textfw "ワラワとの"
	linefw "対戦を のぞむのかえ?"
	done

QueenPlayerQuitText:
	textfw "そうかえ"
	linefw "しかたあるまいのぉ"
	linefw "では このダンジョンから"
	linefw "たちさるが よい"
	done

QueenPlayerWon1Text:
	textfw "ワラワの まけじゃ"
	linefw "みごとな たたかいぶりじゃ"
	linefw "さすが ワラワのところまで"
	linefw "たどりついただけは あるのぉ!"
	linefw "さあ"
	linefw "チップを うけとるのじゃ"
	done

QueenPlayerWon2Text:
	textfw "これで そなたと おわかれとは"
	linefw "さみしい かぎりじゃ"
	linefw "また きてくれることを"
	linefw "ワラワは ねがっておるぞえ"
	done

QueenPlayerLostText:
	textfw "ワラワの 勝ちじゃからな"
	linefw "チップは かえさんぞえ"
	linefw "いまいちど ワラワと たたかいたくば"
	linefw "また くるのじゃな"
	linefw "ポーン ナイト ビショップ ルークと"
	linefw "勝ちぬいて くるのじゃ"
	linefw "さすれば ワラワが いまいちど"
	linefw "対戦するぞえ"
	done

GameCenterCoinFlipAttendantNoPrizesTryAgainText:
	textfw "ざんねんでしたー!!"
	linefw " "
	linefw "3回いじょう 「おもて」がでないと"
	linefw "けいひんは ありませーん"
	linefw "また ちょうせんしてください!"
	done

GameCenterCoinFlipAttendant3HeadsPrizeText:
	textfw "はーい!"
	linefw "「おもて」は 3回でしたぁー"
	linefw "けいひんは"
	linefw "チップ 20枚でーす!"
	done

GameCenterCoinFlipAttendantTryAgainText:
	textfw "また"
	linefw "ちょうせんしてください!"
	done

GameCenterCoinFlipAttendant4HeadsPrizeText:
	textfw "はーい!"
	linefw "「おもて」は 4回でしたぁー"
	linefw "けいひんは"
	linefw "チップ 40枚でーす!"
	done

GameCenterCoinFlipAttendant5HeadsPrizeText:
	textfw "はーい!"
	linefw "「おもて」は 5回でしたぁー"
	linefw "けいひんは"
	linefw "チップ 100枚でーす!"
	done

GameCenterCoinFlipAttendant6HeadsPrizeText:
	textfw "はーい!"
	linefw "「おもて」は 6回でしたぁー"
	linefw "けいひんは"
	linefw "チップ 200枚でーす!"
	done

GameCenterCoinFlipAttendant7HeadsPrizeText:
	textfw "はーい!"
	linefw "「おもて」は 7回でしたぁー"
	linefw "けいひんは"
	linefw "チップ 500枚でーす!"
	done

GameCenterCoinFlipAttendant8HeadsPrizeText:
	textfw "はーい!"
	linefw "「おもて」は 8回でしたぁー"
	linefw "8回なんて すごいですねー!"
	linefw "けいひんは チップ1000枚でーす!"
	done

GameCenterCoinFlipAttendant9HeadsPrizeText:
	textfw "はーい!"
	linefw "「おもて」は 9回でしたぁー"
	linefw "おしかったですね"
	linefw "あと 1回で 10でしたね"
	linefw "けいひんは なんと!"
	linefw "チップ 3000枚でーす!"
	done

GameCenterCoinFlipAttendantAlmostCompleteTryAgainText:
	textfw "10回も ユメじゃないですよ!"
	linefw "また ちょうせんしてください!"
	done

GameCenterCoinFlipAttendant10HeadsPrizeText:
	textfw "おめでとうございまーす!"
	linefw " "
	linefw "「れんぞくコイントス」"
	linefw "10回 れんぞく たっせいでーす!"
	linefw "10回のけいひんは 「ミュウ<Lv>8」!"
	linefw "おうけとりくださーい!!"
	done

GameCenterCoinFlipAttendant10HeadsCongratsText:
	textfw "ほんとーに おめでとーございます!"
	done

TurnedOnBlackBoxText:
	textfw "「ブラックボックス」の"
	linefw "スイッチを いれました"
	done

TurnedOnBillsPCText:
	textfw "「マサキのパソコン」の"
	linefw "スイッチを いれました"
	done

GameCenterBlackBoxAttendantWelcomeText:
	textfw "ようこそいらっしゃいました"
	linefw " "
	linefw "ここのアトラクションは"
	linefw "「ブラックボックス」です"
	linefw "よろしければ かんたんに"
	linefw "ごせつめいいたしますよ"
	done

GameCenterBlackBoxAttendantGuidePromptText:
	textfw "せつめいを ききますか?"
	done

GameCenterBlackBoxAttendantGuideText:
	textfw "「ブラックボックス」は"
	linefw "チップ5枚で あそべます"
	linefw "ポケモンカードを 1枚から5枚"
	linefw "「ブラックボックス」に いれます"
	linefw "すると それらのカードが"
	linefw "べつのカードに だいへんしん!"
	linefw "へんしんしたカードは"
	linefw "メールで あなたに とどきます!!"
	linefw "ですから 「ブラックボックス」を"
	linefw "あそぶときには"
	linefw "メールボックスに あきを"
	linefw "よういしておいて くださいね"
	linefw "メールが たまっていると"
	linefw "カードが とどきませんからね"
	linefw "かんたんですが せつめいは"
	linefw "いじょうで おわりです"
	linefw "それでは 「ブラックボックス」を"
	linefw "おたのしみください"
	done

GameCenterBlackBoxAttendantNoGuideText:
	textfw "せつめいを きかれないのですね"
	linefw "それも よいでしょう"
	linefw "マシンの てじゅんどおりにすれば"
	linefw "あそべますので"
	linefw "また 「ブラックボックス」を"
	linefw "あそぶときには"
	linefw "メールボックスに あきを"
	linefw "よういしておいて くださいね"
	linefw "メールが たまっていると"
	linefw "カードが とどきませんからね"
	done

GameCenterBillsPCAttendantWelcomeText:
	textfw "こんにちは!"
	linefw " "
	linefw "ここのアトラクションは"
	linefw "「マサキのパソコン」っす"
	linefw "よければ カンタンな"
	linefw "せつめいをするっす"
	done

GameCenterBillsPCAttendantGuidePromptText:
	textfw "せつめいを ききますか?"
	done

GameCenterBillsPCAttendantGuideText:
	textfw "では せつめいをするっす"
	linefw " "
	linefw "「マサキのパソコン」は"
	linefw "あたらしいカードを ゲットできるっす"
	linefw "そのためには とくていのカードが"
	linefw "ひつように なるっす"
	linefw "まずは カードの「マサキのパソコン」"
	linefw "これは ぜったい ひつようっす"
	linefw "あと チップも 20枚"
	linefw "ないと あそべないっす"
	linefw "ほかの ひつようなカードは"
	linefw "マシンのほうで しらべてくれっす"
	linefw "「マサキのパソコン」のせつめいは"
	linefw "これで おわりっす"
	done

GameCenterBillsPCAttendantNoGuideText:
	textfw "そうっすか? まあ マシンの"
	linefw "いうとおりにすれば もんだいないっす"
	linefw "ためしに いきなり あそんでみるのも"
	linefw "いいかも しれないっす"
	done

GameCenterCoinFlipAttendantWelcomeText:
	textfw "こんにちは!"
	linefw " "
	linefw "ここのアトラクションは"
	linefw "「れんぞくコイントス」です"
	linefw "コインをなげて 「うら」がでるまで"
	linefw "なんかい 「おもて」をだせるのか?"
	linefw "10回 れんぞく「おもて」なら"
	linefw "「ミュウ<Lv>8」を プレゼント!"
	linefw "「れんぞくコイントス」は"
	linefw "チップ 1枚で あそべます"
	linefw "どうですか?"
	linefw "あそんでみます?"
	done

GameCenterCoinFlipStartPromptText:
	textfw "れんぞくコイントスで あそびますか?"
	done

GameCenterCoinFlipAttendantStartText:
	textfw "それでは 「れんぞくコイントス」の"
	linefw "はじまりでーす!!"
	done

GameCenterCoinFlipAttendantNotEnoughChipsText:
	textfw "もうしわけありませんが"
	linefw "チップを おもちでは ないようですね"
	linefw "「れんぞくコイントス」を あそぶには"
	linefw "チップが 1枚ひつようです"
	done

GameCenterCoinFlipAttendantComeAgainText:
	textfw "それでは またいらしてくださーい!"
	done

GameCenterGRGalText:
	textfw "ああー どれで あそんでいいのか"
	linefw "まよっちゃうわぁー"
	done

GameCenterCoinTossBoyInitialText:
	textfw "「れんぞくコイントス」は"
	linefw "コインをなげて オモテをだすんだ"
	linefw "れんぞく10かい オモテがでると"
	linefw "すごいカードが もらえるんだ"
	done

GameCenterCoinTossBoyRepeatText:
	textfw "ドキドキ つぎは オモテが"
	linefw "でるかなぁ?"
	linefw "オモテがでるか ドキドキして"
	linefw "みもほそる おもいだよー"
	done

GameCenterCoinTossBoyPostgameText:
	textfw "さすがに 「れんぞくコイントス」も"
	linefw "なれたから ドキドキしないや"
	linefw "そろそろ"
	linefw "つぎのゲームを してみようかな?"
	done

GameCenterSlotMachine1AttendantWelcomeText:
	textfw "いらっしゃいましー"
	linefw " "
	linefw "ここのアトラクションは"
	linefw "「スロットマシン」です"
	linefw "こっちの 「スロットマシン」は"
	linefw "1チップで あそべるんだ"
	linefw "よければ かんたんに"
	linefw "せつめいするよん"
	done

GameCenterSlotMachine1AttendantGuidePromptText:
	textfw "せつめいを ききますか?"
	done

GameCenterSlotMachine1AttendantGuideText:
	textfw "せつめいと いっても"
	linefw "むずかしいことは ないよ"
	linefw "スロットマシンで 3つのマークが"
	linefw "そろうと チップを ゲット"
	linefw "レインボーエネルギーが そろうと"
	linefw "ボーナスゲームに とつにゅう!"
	linefw "ここでマークが そろうと なんと!"
	linefw "「プレゼントパック」を プレゼント"
	linefw "さあ おためしあれ!"
	done

GameCenterSlotMachine1AttendantNoGuideText:
	textfw "まあ チップも 1枚だし"
	linefw "ためすと いいよ"
	done

GameCenterSlotMachine5AttendantWelcomeText:
	textfw "はい! こんにちは"
	linefw "「スロットマシン」ですか?"
	linefw "あっちと ちがって こっちは"
	linefw "チップ 5枚が ひつようだよ!"
	linefw "ちょっとした せつめいを するけど"
	linefw "きくきある?"
	done

GameCenterSlotMachine5AttendantGuidePromptText:
	textfw "せつめいを ききますか?"
	done

GameCenterSlotMachine5AttendantGuideText:
	textfw "どっちの 「スロットマシン」も"
	linefw "することは いっしょなんだ"
	linefw "チップをつかって"
	linefw "スロットマシンを うごかす"
	linefw "スロットマシンで 3つのマークを"
	linefw "そろえて チップを ゲット"
	linefw "レインボーエネルギーが そろうと"
	linefw "ボーナスゲームが できる"
	linefw "ボーナスゲームで マークが そろうと"
	linefw "「プレゼントパック」が もらえる"
	linefw "どう? かんたんでしょ"
	linefw "ぜひ やってみてね!"
	done

GameCenterSlotMachine5AttendantNoGuideText:
	textfw "「ひゃくぶんは いっけんに しかず」"
	linefw "あそんで ためすのが イチバン!"
	done

GameCenterCardDungeonAttendantWelcomeText:
	textfw "ようこそ!"
	linefw "カードダンジョンへ!!"
	linefw "あそぶには チップが 10枚"
	linefw "ひつようですけど あそびますか?"
	done

GameCenterCardDungeonStartPromptText:
	textfw "カードダンジョンで あそびますか?"
	done

GameCenterCardDungeonAttendantNotEnoughChipsText:
	textfw "もうしわけありません"
	linefw "チップが たりません"
	linefw "チップを 10枚"
	linefw "もって きてください"
	done

GameCenterCardDungeonAttendantReadDescriptionText:
	textfw "では まず このせつめいぶんを"
	linefw "よんでください"
	done

GameCenterCardDungeonAttendantEnterText:
	textfw "それでは ダンジョンに"
	linefw "おはいりください"
	done

GameCenterCardDungeonAttendantComeAgainText:
	textfw "カードダンジョンで"
	linefw "あそびたくなったら いつでもどうぞ!"
	done

GameCenterCardDungeonAttendantPlayerWonComeAgainText:
	textfw "クィーンさまに 勝ったそうですね"
	linefw "おめでとうございます!"
	linefw "なんどでも あそべますから"
	linefw "また あそびにきてください"
	done

GameCenterCardDungeonAttendantPlayerLostTryAgainText:
	textfw "おつかれさまでした"
	linefw "また あそびに きてください"
	done

GameCenterCardDungeonAttendantPlayerQuitTryAgainText:
	textfw "あれ 対戦を やめたんですか?"
	linefw "よかったら また あそんでください"
	done

GameCenterCardDungeonBoy1Text:
	textfw "オレは"
	linefw "ダンジョンに ちょうせんしてる"
	linefw "チップを かけて カードバトルを"
	linefw "してるんだ! けど勝てない・・・"
	linefw "さいしょの ポーンちゃんにすら"
	linefw "勝てないんだよーーーー!!!!"
	done

GameCenterCardDungeonBoy2Text:
	textfw "ふっふふふ・・・ポーンちゃんには"
	linefw "勝ったんだぜーーーー!!!!"
	linefw "だけど・・・4ばんめのルークさまに"
	linefw "勝てないんだよなぁ"
	linefw "ルークさま つよいしなぁ・・・"
	done

GameCenterCardDungeonBoy3Text:
	textfw "もーダメだぁ!!!"
	linefw "どうやっても 勝てねーーー!!!!"
	linefw "ダンジョンクリアは"
	linefw "もう あきらめたぜ・・・ふぅ・・・"
	done

GameCenterFixerNormalText:
	textfw "いやー しごとで"
	linefw "GR団のキャッスルに きたんだけど"
	linefw "GR団のキングさまが"
	linefw "いそがしいようでね またされてるんだ"
	linefw "こうやって スロットマシンで"
	linefw "じかんを つぶしてるって わけさ"
	done

GameCenterFixerPostgameText:
	textfw "いやー しごとで"
	linefw "GR団のキャッスルに きたんだけど"
	linefw "しごとが なくなっちゃってさ"
	linefw " "
	linefw "スロットマシンで あそんでるしか"
	linefw "ないんだよ まっ しょーがねぇな"
	done

GameCenterSlotMachineGRWomanInitialText:
	textfw "あーちょっと はなしかけないで!"
	linefw "スロットマシンで あそんでるんだから"
	linefw "エネルギーマークが そろうと"
	linefw "チップが もらえるのよ!"
	linefw "しかも レインボーエネルギーマークが"
	linefw "そろうと ボーナスゲーム!"
	linefw "あんたも チップがあるなら"
	linefw "あそんでみたら?"
	done

GameCenterSlotMachineGRWomanWhiffText:
	textfw "あー ちょっと はなしかけないでよ!"
	linefw "マークが そろいそうなんだから・・・"
	linefw "・・・きーー!! はずれたわ!!"
	linefw "あんたが はなしかけたせいよ!"
	linefw "はなしかけて"
	linefw "ジャマしないで ちょうだいよ!"
	done

GameCenterSlotMachineGRWomanHitText:
	textfw "あー ちょっと はなしかけないでよ!"
	linefw "マークが そろいそうなんだから・・・"
	linefw "・・・あーー!! おおアタリ!!"
	linefw "あんたが はなしかけて くれたからよ"
	linefw "これからも バンバン"
	linefw "はなしかけて ちょうだいよ!"
	done

GameCenterChipDeskWelcomeText:
	textfw "いらっしゃいませ"
	linefw "チップカウンターへ ようこそ!"
	linefw "ここでは チップを あずけたり"
	linefw "あずけたチップを うけとれます"
	done

GameCenterChipDeskDepositInfoText:
	textfw "チップは <RAMNUM>枚"
	linefw "あずかっています"
	done

GameCenterChipDeskDepositReturnPromptText:
	textfw "チップを おわたししますか?"
	done

GameCenterChipDeskDepositReturnedText:
	textfw "それでは チップ <RAMNUM>枚"
	linefw "おうけとりください"
	done

GameCenterChipDeskDepositReminderText:
	textfw "ゲームセンターを でるときには"
	linefw "チップを あずけに きてください"
	done

GameCenterChipDeskComeAgainText:
	textfw "またの ごりようを"
	linefw "おまちしております"
	done

GameCenterChipDeskDepositPromptText:
	textfw "チップを あずけますか?"
	done

GameCenterChipDeskDepositedText:
	textfw "たしかに <RAMNUM>枚"
	linefw "おあずかりしました"
	done

GameCenterChipGirlFirstServiceText:
	textfw "いらっしゃいませ!"
	linefw "ゲームセンターに ようこそ!"
	linefw "あなたは チップを おもちでは"
	linefw "ないようですね"
	linefw "それでは チップを 10枚"
	linefw "さしあげましょう"
	done

Received10ChipsText:
	textfw "<RAMNAME>は ゲームセンターの"
	linefw "チップを 10枚 もらった!"
	done

GameCenterChipGirlNoticeText:
	textfw "では ゲームセンターで"
	linefw "おたのしみください"
	linefw "そうそう チップは そとに"
	linefw "もちだせないので ごちゅういを"
	done

GameCenterChipGirlRefillText:
	textfw "あれ? チップを もってないんですね"
	linefw "うーん あなただけに サービスです"
	linefw "チップを 10枚"
	linefw "さしあげましょう!!"
	done

GameCenterChipGirlWelcomeRefillText:
	textfw "こんにちは"
	linefw "ゲームセンターへ ようこそ!"
	linefw "おや"
	linefw "チップを おもちでは ないようですね"
	linefw "サービスです"
	linefw "10枚さしあげましょう"
	done

GameCenterChipGirlEnjoyText:
	textfw "では ゲームセンターで"
	linefw "おたのしみください"
	done

GameCenterChipGirlRefillReminderText:
	textfw "チップが なくなったら"
	linefw "また きてください"
	linefw "10枚だけですが チップを"
	linefw "さしあげますよ"
	done

GameCenterChipSecurityReminderText:
	textfw "チップは ゲームセンターから"
	linefw "もちだせません"
	linefw "かならず ひだりの カウンターで"
	linefw "あずけてください"
	done

GameCenterChipSecurityWarningText:
	textfw "しつれい おまちいただけますか?"
	linefw "あなたさまは チップを おもちですね"
	linefw "もうしわけありませんが チップは"
	linefw "ゲームセンター せんようです"
	linefw "そとに おでになるのであれば"
	linefw "チップを あずけてからにしてください"
	linefw "おてすうですが"
	linefw "よろしくおねがい いたします"
	done

GameCenterTech20YearLongPoorGamblerText:
	textfw "ここの ゲームの研究をして"
	linefw "くせつ20ねん・・・"
	linefw "いまだに けっかを みちびきだせない"
	linefw "まけ まけ! まけ!! まけつづけ!"
	linefw "ようは・・・"
	linefw "あそんでるだけ なんですけどね"
	done

GameCenterGRLassMockeryText:
	textfw "あなた <RAMNAME>くんよね"
	linefw "ここで なにしてるの"
	linefw "ははぁ GR団に 勝てないから"
	linefw "ここで あそんでるのね"
	linefw "あー かっこうわるぅーい!"
	done

GameCenterGRLassNoWordsToVictorOverBiruritchiText:
	textfw "ビルリッチさまが まけたんだから"
	linefw "なにしてても モンクないわ"
	done

GameCenterGRPappyLectureText:
	textfw "ゲームに ムチュウになって"
	linefw "じかんを わすれて あそんどった"
	linefw "ほしたら こーんなに"
	linefw "トシを とってしもうたわい・・・"
	linefw "もちろん ウソじゃ"
	linefw " "
	linefw "ゲームに ムチュウになって"
	linefw "ほんとのもくてきを わすれんようにな"
	done

GameCenterGRPappyPlayFreelyText:
	textfw "もくてきを はたしたようじゃな"
	linefw "あとは すきに あそぶが ええ"
	done

Text0d8c:
	textfw "ようこそ いらっしゃいました"
	linefw "ここは 草のGR団のトリデです"
	done

Text0d8d:
	textfw "ようこそ いらっしゃいました"
	linefw "ここは 草のGR団のトリデです"
	linefw "草のGR団の 対戦のオキテを"
	linefw "ごせつめいいたします"
	linefw "草のGR団のトリデの リーダーは"
	linefw "モリノさまと もうします"
	linefw "モリノさまに あうには 草のGR団員"
	linefw "3にんに 勝たねばなりません"
	linefw "なかには じょうけんつきで"
	linefw "対戦をする団員も おります"
	linefw "では 草のGR団のトリデの対戦を"
	linefw "おたのしみください"
	done

Text0d8e:
	textfw "<RAMNAME>くん クルシマです"
	linefw "サイエンスクラブマスターの"
	linefw "どうして GR団のしまに?"
	linefw " "
	linefw "えっ カードを とりもどしに?"
	linefw "GR団に ちょうせんされて・・・?"
	linefw "そうですか・・・"
	linefw "いいことを おしえましょう"
	linefw "さきに すすむには コインが"
	linefw "ひつように なるようです"
	linefw "コインは それぞれのトリデの"
	linefw "リーダーが もっているようです"
	linefw "リーダーとの対戦で 勝たないと"
	linefw "コインは てに はいらないようです"
	linefw "でも グランドマスターに勝った"
	linefw "あなたなら だいじょうぶです"
	linefw "どうか みんなのためにも"
	linefw "がんばってください"
	done

Text0d8f:
	textfw "GR団には"
	linefw "キングが いるそうです"
	linefw "GR団のキングを たおさないと"
	linefw "へいわは こないでしょう"
	linefw "あなたなら GR団のキングをも"
	linefw "たおせるでしょう"
	linefw "どうか みんなのためにも"
	linefw "がんばってください"
	done

Text0d90:
	textfw "こんにちはー"
	linefw "<RAMNAME>ちゃんでしょー?"
	linefw "あたし ミドリって いうの"
	linefw "草のGR団員だよ"
	linefw "ずっとね <RAMNAME>ちゃんが"
	linefw "くるの まってたんだよー!"
	linefw "だって GR団じゃない あいてと"
	linefw "対戦したかったんだもーん!"
	linefw "対戦してくれるでしょ?"
	linefw "勝ったら 拡張パックも あげるし"
	linefw "勝ったら あそこのおニイさん"
	done

Text0d91:
	textfw "たしか サイエンスクラブのマスター"
	linefw "クルシマさん・・だっけ?"
	done

Text0d92:
	textfw "かえして あげるから"
	done

Text0d93:
	textfw "ねー 対戦しようよー"
	linefw " "
	linefw "どーせ 対戦しないと"
	linefw "さきに いけないんだからさー"
	done

Text0d94:
	textfw "ミドリと 対戦しますか?"
	done

Text0d95:
	textfw "じゃあ サイドカードは 4枚ね"
	linefw "たのしみだなー"
	done

Text0d96:
	textfw "えー? だって 対戦しないと"
	linefw "さきに いけないんだよー"
	linefw "他のGR団のトリデに いきたかったら"
	linefw "対戦しようねー"
	done

Text0d97:
	textfw "あたし <RAMNAME>ちゃんと"
	linefw "ポケモンカードで あそびたいなー"
	linefw "ねぇ ねぇ 対戦しようよー"
	done

Text0d98:
	textfw "サイドカードは 4枚で"
	linefw "ゆっくり あそぼうね"
	done

Text0d99:
	textfw "えー つまんなーい"
	linefw "・・・でも しょうがないや"
	linefw "また 対戦に きてね"
	linefw "あたし まってるから"
	done

Text0d9a:
	textfw "あやや まけちゃった"
	done

Text0d9b:
	textfw "まけても くやしくないや"
	linefw "なんでだろ?"
	linefw "そっか デッキが いいから"
	linefw "まけたんだもんね なっとく"
	done

Text0d9c:
	textfw "もう おわっちゃった・・・"
	linefw "また 対戦に きてね"
	done

Text0d9d:
	textfw "あれ? また まけちゃった"
	linefw "んー なんでだろ?"
	done

Text0d9e:
	textfw "よーし! つぎの対戦じゃ"
	linefw "まけないぞー"
	linefw "また 対戦してね"
	linefw "あたし まってるから"
	done

Text0d9f:
	textfw "やったー! 勝っちゃったー"
	linefw "うれしーなっ!"
	linefw "つぎも 対戦しに きてね!"
	done

Text0da0:
	textfw "じゃあ クルシマさんを"
	linefw "オリからだして あげるね"
	done

Text0da1:
	textfw "たすけてくれて ありがとう"
	linefw "<RAMNAME>くん"
	linefw "おれいに このカードを"
	linefw "うけとってください"
	done

Text0da2:
	textfw "これからの 対戦に"
	linefw "うまく つかってください"
	linefw "わたしは サイエンスクラブに"
	linefw "もどることにします"
	linefw "こんど サイエンスクラブに"
	linefw "きてください"
	linefw "わたしと 対戦しましょう"
	linefw "それでは しつれいします"
	done

Text0da3:
	textfw "はい これで ユウタくんのヘヤに"
	linefw "いけるから"
	linefw "じゃあ また 対戦しようね"
	done

Text0da4:
	textfw "あら もう きたん?"
	linefw "ユウタくんも だらしないなぁ"
	linefw "うちは ミユキや"
	linefw "もちろん GR団員や"
	linefw "きみ GR団に さからっとる"
	linefw "<RAMNAME>くん やろ?"
	linefw "モリノはんが もっとる"
	linefw "「草のGR団コイン」が ほしいんやろ"
	linefw "それ ないと さきに"
	linefw "すすまれへんもんな"
	linefw "ほなら ちゃっちゃと 対戦しょ!"
	linefw "うち はなのていれ したいんや"
	linefw "ただし うちとの対戦は"
	linefw "じょうけんつきや"
	linefw "デッキに 草エネルギーいがいの"
	linefw "エネルギーを いれたら アカン"
	linefw "これが うちと 対戦する"
	linefw "じょうけんや"
	linefw "どないするん <RAMNAME>くん?"
	done

Text0da5:
	textfw "んもう また きたん?"
	linefw "ええかげんに してほしいわぁ"
	linefw "うちかて あそんどるわけや ないんよ"
	linefw " "
	linefw "キングはんの めいれいも あるさかい"
	linefw "つよいヤツの あいては するけどね"
	linefw "対戦のじょうけんは おぼえとる?"
	linefw "エネルギーは 草エネルギーだけや"
	linefw "デッキに 草エネルギーいがいの"
	linefw "エネルギーを いれたら あかん"
	linefw "そんで どないや? <RAMNAME>くん"
	linefw "対戦するんやろね?"
	done

Text0da6:
	textfw "ミユキと 対戦しますか?"
	done

Text0da7:
	textfw "あー あかん!"
	linefw "草エネルギーだけや いうたやろ"
	linefw "草エネルギーいがいの"
	linefw "エネルギーが はいっとるやないの"
	linefw "デッキにいれて ええのは"
	linefw "草エネルギーだけや"
	linefw "草エネルギーだけ いれて"
	linefw "デッキ くみなおして きぃや"
	done

Text0da8:
	textfw "うん ちゃんと 草エネルギーだけで"
	linefw "デッキを くんどるね"
	linefw "ほしたら サイドカード 4枚で"
	linefw "対戦 スタートや!"
	done

Text0da9:
	textfw "草エネルギーが たらへんのやったら"
	linefw "あつめてから きぃや"
	linefw "GR団のしまから かえるのも"
	linefw "ひとつの てやと おもう"
	linefw "なんにせよ 草エネルギーだけで"
	linefw "デッキを くんでから くるんやな"
	done

Text0daa:
	textfw "じょうけんつきで"
	linefw "対戦しとうなったんか?"
	linefw "もう わかっとると おもうけど"
	linefw "じょうけんを はなすで"
	linefw "草エネルギーだけが はいっとる"
	linefw "デッキやないと 対戦せぇへん"
	linefw "ほな 対戦しよか"
	done

Text0dab:
	textfw "うちのはなし ちゃんと"
	linefw "きいとったん?"
	linefw "いれていい エネルギーは「草」だけや"
	linefw "わすれたら あかんよ"
	linefw "また デッキを くみなおしたら"
	linefw "対戦に きぃや"
	done

Text0dac:
	textfw "草エネルギーだけで デッキを"
	linefw "くんどるようやね これなら ええで"
	linefw "はなのていれも あるさかいに"
	linefw "サイドカードは 4枚や"
	linefw "ちゃっちゃと 対戦しようやないの"
	done

Text0dad:
	textfw "じょうけんが きにいらへんかったら"
	linefw "対戦は せぇへんよ"
	linefw "ごうに いっては ごうに したがわな"
	linefw "あかんことを おぼえとくんやな"
	done

Text0dae:
	textfw "あら まけて もうたやないか"
	done

Text0daf:
	textfw "<RAMNAME>くんは ええ うでやな"
	linefw "おどろいたわ"
	done

Text0db0:
	textfw "はい! さよなら"
	linefw "うちは はなのていれが あるさかい"
	linefw "草エネルギーだけの デッキを"
	linefw "ようけ つかえるようなったら きぃや"
	linefw "うちは にげも かくれもせんから"
	done

Text0db1:
	textfw "じょうけんつきの 対戦にも"
	linefw "じゅうぶん なれたようやね"
	done

Text0db2:
	textfw "じょうけんつきで ええなら"
	linefw "また 対戦しよな"
	done

Text0db3:
	textfw "まだ まだやね うまく デッキを"
	linefw "つかえるようにならんと あかんよ"
	done

Text0db4:
	textfw "うちに 勝ったんやから"
	linefw "モリノはんに あう しかく ありや"
	linefw "ちょっと まっとき"
	done

Text0db5:
	textfw "おくに いけば モリノはんに"
	linefw "あえるさかい さっさと いきぃな"
	linefw "うちは はなのていれが あるんや"
	linefw "これいじょう じゃませんといてや"
	done

Text0db6:
	textfw "よーこそー 草のGR団のリーダー"
	linefw "モリノの にわへ"
	linefw "ちなみに ぼくが モリノだよー"
	linefw "きみは <RAMNAME>くんだねー"
	linefw "ぼくの 「草のGR団コイン」"
	linefw "これが ほしーんだよねー"
	linefw "そーだなー やっぱり 対戦かなー"
	linefw "対戦して 勝ったら あげるよー"
	linefw "そーそー おしえとくねー"
	linefw "ここでは とくべつな対戦なんだよー"
	linefw "草ポケモンはー 「どく」や「マヒ」や"
	linefw "「こんらん」や「ねむり」"
	linefw "どーれにも ならないんだよー"
	linefw "すごいだろー"
	linefw "ここは そーいう ところだけど"
	linefw "それでも 対戦するー?"
	done

Text0db7:
	textfw "やー <RAMNAME>くん"
	linefw "ようこそー ぼくの にわへ"
	linefw "こんな しぜんに かこまれて"
	linefw "対戦するなんて しあわせだよねー"
	linefw "ぼくのにわは 草ポケモンが"
	linefw "げんき たくさん いっぱいだからねー"
	linefw "草ポケモンはー 「どく」や「マヒ」や"
	linefw "「こんらん」や「ねむり」"
	linefw "どれにも ならないからねー"
	linefw "わかってるよねー"
	linefw "それじゃー 対戦しようかー"
	linefw " "
	linefw "勝てば 「草のGR団コイン」を"
	linefw "ゲットできるんだよー"
	done

Text0db8:
	textfw "モリノと 対戦しますか?"
	done

Text0db9:
	textfw "ぼくはー ゆっくり 対戦したいんだー"
	linefw "だから サイドカードは 6枚ねー"
	done

Text0dba:
	textfw "対戦を ことわるってことはー"
	linefw "どくや マヒが いっぱいなんだねー"
	linefw "それじゃー デッキを"
	linefw "つくりなおしておいでー"
	linefw "ぼくはー ここで まってるからー"
	linefw "ああ あわてなくてもー いいからねー"
	done

Text0dbb:
	textfw "GR団はねー デッキを かえて"
	linefw "対戦できると そんけーされるんだよー"
	linefw "ぼくと ミユキちゃんに 勝ったから"
	linefw "きみを みとめてるよー"
	linefw "そんな そんけーされる"
	linefw "きみと 対戦したいんだー"
	linefw "けど ぼくのにわは みどりのちからー"
	linefw "草ポケモン げんき いっぱーい"
	linefw "草ポケモンが 「どく」や 「マヒ」"
	linefw "「こんらん」「ねむり」"
	linefw "どれにもならない対戦だー"
	done

Text0dbc:
	textfw "ぼくは デッキを つかいこなしてる"
	linefw "きみを そんけーしてるよー"
	linefw "ぼくのにわは 草ポケモンが"
	linefw "げんき いっぱいだからねー"
	linefw "草ポケモンはー 「どく」や「マヒ」や"
	linefw "「こんらん」「ねむり」"
	linefw "どれにも ならないんだぞー"
	linefw " "
	linefw "そんな ふしぎな ぼくのにわで"
	linefw "いいならー 対戦するかい?"
	done

Text0dbd:
	textfw "サイドカード 6枚で"
	linefw "じっくり ゆっくり 対戦しましょー"
	done

Text0dbe:
	textfw "ぼくと 対戦するときは"
	linefw "勝ち まけは ムシしましょー"
	linefw "やっぱり ポケモンカードは"
	linefw "きもちよく あそばないとねー"
	done

Text0dbf:
	textfw "キングさまにも 勝ったんだってー?"
	linefw "おそれいりましたねー"
	linefw "カードは デッキに つかうだけでも"
	linefw "ダメな ようですねー"
	linefw "やっぱり たのしくあそぶのが"
	linefw "いちばん いーのでしょうねー"
	done

Text0dc0:
	textfw "あははははは・・・・"
	linefw "まけて しまったようだねー"
	done

Text0dc1:
	textfw "まけるのも べんきょうに"
	linefw "なるんだよー しってたかい?"
	linefw "まけると どんな対戦か"
	linefw "よーく わかるだろー"
	done

Text0dc2:
	textfw "あははははは おめでとー"
	linefw "さすが <RAMNAME>くんだねー"
	done

Text0dc3:
	textfw "きもちよく あそべましたかー?"
	linefw "よければ また 対戦しましょー"
	done

Text0dc4:
	textfw "きみに 勝ってもらったほうが"
	linefw "うれしーような きがします"
	linefw "やっぱり 勝ったときの きみの"
	linefw "えがおが すてきだからねー"
	done

Text0dc5:
	textfw "あーそうかー 勝ったんだから"
	linefw "「草のGR団コイン」 あげないとねー"
	done

Text0dc6:
	textfw "これで 雷のGR団のトリデに"
	linefw "はいれるように なったねー"
	linefw "しぜん いっぱいのなかで"
	linefw "対戦したければ また おいでー"
	done

Text0dc7:
	textfw "ぼくは GRだーん!"
	linefw "つよいぞ・・・あれ?"
	linefw "みたことのない カオだね"
	linefw "きみなら もってるかなぁ?"
	linefw "ぼく 「わるいフシギバナ<Lv>37」が"
	linefw "ほしーんだよね"
	linefw "どう? ぼくの「フシギバナ<Lv>67」と"
	linefw "こうかんしない?"
	linefw "ただの「フシギバナ<Lv>67」じゃないぜ"
	linefw "イラストがちがう めずらしいやつだ!"
	done
